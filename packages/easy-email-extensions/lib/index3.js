...
function runHandlers(map, event, view, scope) {
  ...
  if (scopeObj) {
    if (runFor(scopeObj[prefix + modifiers(name2, event, !isChar)])) {
      handled = true;
    } else if (isChar && (event.altKey || event.metaKey || event.ctrlKey) && !(browser.windows && event.ctrlKey && event.altKey) && !(browser.mac && event.altKey && !(event.ctrlKey || event.metaKey)) && (baseName = base[event.keyCode]) && baseName != name2) {
...
function defaultPositionInfo(view, list, option, info, space2, tooltip) {
  ...
  if (infoWidth <= (left ? spaceLeft : spaceRight)) {
    ...
    narrow = true;
    maxWidth = Math.min(400, (rtl ? list.right : space2.right - list.left) - 30);
...
var basicSetup = function basicSetup2(options) {
  ...
  if (options.syntaxHighlighting !== false)
    extensions2.push(syntaxHighlighting(defaultHighlightStyle, {
      fallback: true
    }));
  ...
  if (initCrosshairCursor !== false)
    extensions2.push(crosshairCursor());
  ...
  if (options.tabSize && typeof options.tabSize === "number")
    extensions2.push(indentUnit.of(" ".repeat(options.tabSize)));
  return extensions2.concat([keymap.of(keymaps.flat())]).filter(Boolean);
};
...
function useCodeMirror(props) {
  var {
    ...
    onCreateEditor,
    onUpdate,
    extensions: extensions2 = emptyExtensions,
    autoFocus,
    theme: theme2 = "light",
    height = null,
    minHeight = null,
  ...
  getExtensions = getExtensions.concat(extensions2);
  useLayoutEffect(() => {
    if (container && !state) {
      var config2 = {
        doc: value,
  ...
  }, [theme2, extensions2, height, minHeight, maxHeight, width, minWidth, maxWidth, placeholderStr, editable2, readOnly2, defaultIndentWithTab, defaultBasicSetup, onChange, onUpdate]);
  useEffect(() => {
    if (value === void 0) {
      return;
    }
...
var CodeMirror = ReactCodeMirror;
class Stack {
